...
const Check = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 text-gray-400 ml-auto shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
...
function PriorityPicker({ value, onChange }) {
  ...
  const pr = priorityMeta(value);
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl border border-gray-200 bg-white hover:border-gray-300 transition-colors"
      >
        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ${pr.bg} ${pr.text}`}>{pr.label}</span>
        <Chevron />
      </button>
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
          {PRIORITIES.map((p) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 hover:bg-gray-50 transition-colors ${value === p.value ? 'bg-gray-50' : ''}`}
            >
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ${p.bg} ${p.text}`}>{p.label}</span>
              {value === p.value && <Check />}
            </button>
          ))}
        </div>
...
function ColumnPicker({ value, onChange }) {
  const [open, setOpen] = useState(false);
  const col = COLUMNS.find((c) => c.key === value) ?? COLUMNS[0];
  return (
    <div className="relative">
      <button
        type="button"
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
          {COLUMNS.map((c) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 hover:bg-gray-50 transition-colors ${value === c.key ? 'bg-gray-50' : ''}`}
            >
              <span className={`w-2 h-2 rounded-full shrink-0 ${c.color}`} />
              <span className="text-[13px] font-medium text-gray-800">{c.label}</span>
              {value === c.key && <Check />}
            </button>
          ))}
        </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <PriorityPicker value={form.priority} onChange={(v) => setForm((f) => ({ ...f, priority: v }))} />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Column</label>
                  <ColumnPicker value={form.status} onChange={(v) => setForm((f) => ({ ...f, status: v }))} />
                </div>
              </div>
