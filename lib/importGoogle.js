...
// 4×4 grid focused on central Warsaw (Śródmieście + surrounding districts)
// Tight bbox avoids wasting cells on suburbs with few businesses
const GRID_CELLS = (() => {
  const cells = [];
  const ROWS = 4, COLS = 4;
  const LAT_MIN = 52.15, LAT_MAX = 52.32; // central Warsaw lat range
  const LNG_MIN = 20.91, LNG_MAX = 21.15; // central Warsaw lng range
...
const SEARCH_RADIUS = 3000; // meters — ~550m overlap between cells
