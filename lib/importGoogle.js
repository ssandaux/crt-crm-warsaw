...
import { enrichEmailsForIds } from './enrichEmails';
...
export async function importFromGoogle(map, onProgress) {
  ...
  // Auto-enrich emails for newly inserted businesses (fire-and-forget, progress via onProgress)
  if (insertedIds.length > 0) {
    enrichEmailsForIds(insertedIds, (p) =>
      onProgress({
        step: null,
        total: null,
        label: 'Fetching emails',
        status: `${p.done}/${p.total} sites scanned, ${p.found} emails found`,
        totalInserted,
      })
    ).catch(() => {});
  }
